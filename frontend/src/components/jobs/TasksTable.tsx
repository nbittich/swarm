    ...
    const restartTask = async (task: Task) => {
        ...
        try {
            const response = await axios.post(
                `/api/jobs/${task.jobId}/tasks/${task._id}`,
            );
        ...
        },
        {
            ...
            render: (_, record) => {
                if (record.result) {
                    ...
                    if (
                        res.type == "diff" ||
                        res.type == "extractRDFa" ||
                        res.type == "filterSHACL" ||
                        res.type == "scrapeWebsite" ||
                        res.type == "complementWithUuid"
                    ) {
                        return <>{res.value.successCount}</>;
                    }
                }
                return "N/A";
            },
                            ...
                            <>
                                <Tag>
                                    <Link
                                        disabled={!token}
                                        onClick={async () =>
                                            await download(record.jobId, res.value.manifestFilePath)
                                        }
                                    >
                                        <DownloadOutlined />
                                        manifest.json
                                ...
                                <Tag>
                                    <Link
                                        ...
                                        onClick={async () =>
                                            await download(
                                                record.jobId,
                                                res.value.insertedTripleFilePath,
                                            )
                                        }
                                    >
                                        <DownloadOutlined />
                                ...
                                <Tag>
                                    <Link
                                        ...
                                        onClick={async () =>
                                            await download(
                                                record.jobId,
                                                res.value.removedTripleFilePath,
                                            )
                                        }
                                    >
                                        <DownloadOutlined />
                                ...
                                <Tag>
                                    <Link
                                        ...
                                        onClick={async () =>
                                            await download(
                                                record.jobId,
                                                res.value.intersectTripleFilePath,
                                            )
                                        }
                                    >
                                        <DownloadOutlined />
            ...
            render: (status: Status) => {
                return (
                    <>
                        <Tag
                            title={
                                status.type == "failed" ? status.value.join(", ") : undefined
                            }
                ...
                if (record.hasSubTask) {
                    return (
                        <Button
                            ...
                            onClick={() =>
                                navigate(
                                    `/jobs/${record.jobId}/tasks/${record._id}/${record.name}`,
                                )
                            }
        ...
        },
        {
            ...
            render: (_, record) => {
                return (
                    <>
                        <Button
                            disabled={record.status.type !== "failed" || !token}
                            type="link"
