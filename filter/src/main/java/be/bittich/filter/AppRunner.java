          ...
          if (task.status() instanceof Status.Scheduled
            ...
            Thread.startVirtualThread(() -> {
              ...
              var newTask = task.toBuilder()
                  ...
                  .modifiedDate(Optional.of(OffsetDateTime.now(ZoneOffset.UTC)))
              ...
              try {
                ...
                res.ifPresent(
                    updatedTask -> nc.publish(Constant.TASK_STATUS_CHANGE_EVENT.apply(updatedTask.id()), updatedTask));
              } catch (Exception ex) {
                log.debug("error {}", ex);
    ...
    if (task.payload() instanceof Payload.FromPreviousStep fps
      ...
      try (var manifestReader = new BufferedReader(new FileReader(payload.manifestFilePath().toFile()))) {
        ...
        for (var entry : handles.entrySet()) {
          SubTask subTask = entry.getKey().toBuilder().modifiedDate(Optional.of(OffsetDateTime.now(ZoneOffset.UTC)))
              .build();
          try {
            var res = entry.getValue().get();
            if (res.len() == 0) {
              subTask = subTask.toBuilder()
                  .status(Status.Failed.builder().value(List.of("filtered all data")).build()).build();
              failureCount += 1;
            } else {
        ...
        task = task.toBuilder().modifiedDate(Optional.of(OffsetDateTime.now(ZoneOffset.UTC))).build();
        if (successCount == 0 && failureCount > 0) {
          task = task.toBuilder().status(Status.Failed.builder()
              .value(
                  List.of(String.format("task did not succeed: success: %s, failure: %s", successCount, failureCount)))
              .build()).build();
        } else {
          task = task.toBuilder()
              .result(Optional.of(
                  TaskResult.FilterSHACL.builder()
                      .value(
                          FilterSHACLValue.builder()
                              ...
                              .manifestFilePath(task.outputDir().resolve(Constant.MANIFEST_FILE_NAME))
                              .build())
                      .build()))
